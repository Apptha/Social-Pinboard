 ...
 * @abstract      : Social Pinboard Component follow model
...
class socialpinboardModelfollow extends SocialpinboardModel {
    ...
    function getFollowing() {
        ...
        $query = "SELECT b.user_id, b.username,b.first_name,b.last_name,b.user_image,a.follow_user_board, (
                SELECT COUNT( board_id )
                FROM #__pin_boards
                WHERE (user_id = b.user_id OR cuser_id=b.user_id) and status=1
                ) AS bid, (
                SELECT COUNT( pin_id )
                FROM #__pin_pins
                ...
                INNER JOIN #__pin_user_settings AS b ON a.follow_user_id = b.user_id
                WHERE a.user_id =$userId
        AND b.status=1 group by b.user_id";

        $db->setQuery($query);
        $follow_details = $db->loadObjectList();
    ...
    function getFollower() {
        ...
        $query = "SELECT b.user_id,b.username,b.first_name,b.last_name,b.user_image,a.follow_user_board,  (SELECT COUNT( board_id )
                FROM #__pin_boards
                WHERE (user_id = b.user_id OR cuser_id=$userId) and status=1
                ) AS bid, (
    ...
    function countPins() {
        ...
        $query = "select count(pin_id) from #__pin_pins where status=1 and pin_user_id=" . $userId;
        $db->setQuery($query);
    ...
    function totalBoard() {
        ...
        $query = "select count(board_id)
                  from #__pin_boards where status=1 and (cuser_id=$userId OR user_id= $userId)";
        $db->setQuery($query);
        $total = $db->loadResult();
...
on a.user_id=b.pin_user_id where b.pin_id = $resultmodrepin AND a.status = 1";
    ...
    static function Userboardscount($userid, $fuserid) {
        $db = JFactory::getDBO();
        $query = "SELECT count(board_id)
                  FROM #__pin_boards
                  WHERE status=1
                  AND (user_id=" . $fuserid . " OR cuser_id LIKE '%$fuserid%')";
        $db->setQuery($query);
        $fuser_board_count[0] = $db->loadResult();

        $query = "SELECT follow_user_board
FROM  `#__pin_follow_users`
WHERE user_id =  '$userid'
AND follow_user_id =  '$fuserid'";
        $db->setQuery($query);
        ...
        $user_following_boards[0] = count($boards);
        $result_board_count = array_merge($fuser_board_count, $user_following_boards);
        return $result_board_count;
    }
...
b.user_id=a.pin_user_id where (a.pin_user_id=$userId  AND b.status=1 and a.status = 1
    ...
    static function followUser($user, $fuser) {
