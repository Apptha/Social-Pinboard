...
?><div class="invite_frnds" style="height:600px;" >
    ...
    <?php if (empty($code)) {
 ?><ul><li>
                        <a class="gmail_invite_btn" href="https://accounts.google.com/o/oauth2/auth?client_id=<?php echo $gmailinvite->setting_gmailclientid; ?>&redirect_uri=<?php echo $base_url . JRoute::_('index.php?option=com_socialpinboard&view=invitefriends&task=gmail', false) ?>&scope=https://www.google.com/m8/feeds/&response_type=code" >
                        <img src="<?php echo JURI::base() . '/components/com_socialpinboard/images/gmail-btn.png'; ?>" alt=""/>
    ...
    ?>
            ...
            <?php
                ...
                foreach ($result as $title) {
                    if (!empty($xml->entry[$i]->title[0]))
                        $gmailuser_name = $xml->entry[$i]->title[0];
                    else
                        $gmailuser_name=$title->attributes()->address;

                    $button_eve = "Modal.show('EmailModal'); binddata('" . $gmailuser_name . "','" . $title->attributes()->address . "'); return false";
                    echo '<li class="friend"><div class="individual email-id">
        <div class="name">
            ' . $gmailuser_name . '
        </div>
        <div class="email">
            <div class="email">' . $title->attributes()->address . '</div>
        </div>
    </div>
