...
<?php } ?>
                ...
                ?>
                                <a  href="<?php echo JRoute::_("index.php?option=com_socialpinboard&view=follow&follower=1&uid=" . $userId, false); ?>" ><?php echo count($followings) . ' ' . JText::_('COM_SOCIALPINBOARD_FOLLOWING_CAPS'); ?></a>     <a href="<?php echo JRoute::_("index.php?option=com_socialpinboard&view=follow&follower=0&uid=" . $userId, false); ?>" ><?php echo count($followers) . ' ' . JText::_('COM_SOCIALPINBOARD_FOLLOWERS_CAPS'); ?></a>
                <?php } else { ?>
...
?>
    ...
    ?>
                ...
                <?php echo 'Boards: ' . $following->bid.' '; ?>
                <?php echo 'Pins: ' . $following->pid; ?>
                                    </div>
                                </div>
        <?php
                                        ...
                                        if ($following->user_id != $user->id && $user->id != '' && $following->user_id != '') {
                                            if (($resultuserfollow == 'all') || ($totalBoard == $follwercount)) {
        ?>
                                                <input type="button" name="unfollow" value="<?php echo JText::_('COM_SOCIALPINBOARD_UNFOLLOW'); ?>" id="unfollow" class="unfollow_btn" onclick="unfollowall(<?php echo $user->id . ',' . $following->user_id; ?>,'<?php echo $following->follow_user_board; ?>');" />
        <?php
                                            } else {
        ?><div id="followalldiv">
                                                    <input type="button" name="followall" value="<?php echo JText::_('COM_SOCIALPINBOARD_FOLLOW'); ?>" id="followall" onclick="followall(<?php echo $user->id . ',' . $following->user_id; ?>,'<?php echo $following->follow_user_board; ?>');"/>
                                                </div> <?php
                                            }
                                        } else {
        ?>
    ...
    ?>
        ...
        <?php
                                        ...
                                        if ($follower->user_id != $user->id && $user->id != '' && $follower->user_id != '') {
                                            if (($resultuserfollow == 'all') || ($Userboardscount[0] == $Userboardscount[1])) {
        ?>
                                                <input type="button" name="unfollow" value="<?php echo JText::_('COM_SOCIALPINBOARD_UNFOLLOW'); ?>" id="unfollow" class="unfollow_btn" onclick="unfollowall(<?php echo $user->id . ',' . $follower->user_id; ?>,'<?php echo $follower->follow_user_board; ?>');" />
        <?php
                                            } else {
        ?><div id="followalldiv">
                                                    <input type="button" name="followall" value="<?php echo JText::_('COM_SOCIALPINBOARD_FOLLOW'); ?>" id="followall" onclick="followall(<?php echo $user->id . ',' . $follower->user_id; ?>,'<?php echo $follower->follow_user_board; ?>');"/>
                                                </div> <?php
                                            }
                                        } else {
        ?>
