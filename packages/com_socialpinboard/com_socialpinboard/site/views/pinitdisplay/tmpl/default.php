...
$resultboards = $this->boardname;
$url = JRequest::getVar('url');
$currency = $this->currency;
?>

<script type="text/javascript">
     var enter_board_name = "<?php echo JTEXT::_('COM_SOCIALPINBOARD_PLEASE_ENTER_BOARD_NAME'); ?>";
var board_name_exist = "<?php echo JTEXT::_('MOD_SOCAILPINBOARD_MENU_BOARD_NAME_EXIST'); ?>";
    ...
    scr(document).ready(function() {
        FancyForm.setup();
        Tagging.initTextarea('#Bookmarklet .DescriptionTextarea');
        Tagging.priceTag("#Bookmarklet .DescriptionTextarea", "#Bookmarklet .Images");

    });
</script>
<style type="text/css" media="screen"> 
    #Bookmarklet {width: 550px; margin: 40px auto 0 5%;}
    .FancyForm .fff {position: static ! important;}
    .PinForm {margin-left: 0px ! important;}
    .upload_pin_window #pin_desc {width: 100% !important;}
    body{background: #F7F5F5;font-family: "helvetica neue",arial,sans-serif;}
</style> 
<?php
if ($user->id == 0) {
    $uri = JFactory::getURI();
    $mediaexplo = explode('&', $uri);
    $media = $mediaexplo[2];
    $mediexplo = explode('=', $media);
    $medurl = $mediexplo[1];
    $return = $uri->toString();
    $url = "index.php?option=com_socialpinboard&view=people";
    $url .= '&media=' . urlencode($medurl) . '&returnURL=' . base64_encode($return);
    $linkurl = JRoute::_($url);
    $redirectUrl = $app->redirect($linkurl);
} else {
    $mediaurl = JRequest::getVar('media');
    if (strstr($mediaurl, "http://")) {
        $mediaurl = $mediaurl;
    } else {
...
?>
    ...
    <div id="Bookmarklet">
        <div id="AddForm">
            ...
            <form action="" method="post" id="bookmarklet_addpin_form" name="frmUrl1">
                <div class="image-left">
                        ...
                        <div class="Images pin">
                            ...
                            <ul>
                                ...
                                <li>
                                    ...
                                    <img src="<?php echo $mediaurl; ?> " style=" width: 170px; "/></li>
                            </ul>
                        </div>
                    ...
                    <ul id="upload_pin" class="upload_pin_window" style="display:block;">
                        <li class="custom-select_window">
...
<?php $i++;
                    ...
                    <li class="select-box" style="padding-top: 8px;">
                        <input type="hidden" name="board_selection" id="board_selection"  value="<?php echo $boardid; ?>"/>
                        <div class="uploadtext">
                            <input style="color:#C9C8C8" class="select-board" type="text" name="uploadboardtxt" id="uploadboardtxt" onFocus="onFocusMenu(this,'<?php echo JTEXT::_('COM_SOCIALPINBOARD_PLEASE_ENTER_BOARD_NAME') ?>');" onBlur="onBlurEvent(this,'<?php echo JTEXT::_('COM_SOCIALPINBOARD_PLEASE_ENTER_BOARD_NAME') ?>');" value="<?php echo JTEXT::_('COM_SOCIALPINBOARD_PLEASE_ENTER_BOARD_NAME') ?>" />
<!--                                <input type="text" class="select-board" name="uploadboardtxt" id="uploadboardtxt" value=""/>-->
                        </div>
                        <input type="button" class="creat_bttn" onclick="addnewmenuboard(<?php echo $user->id; ?>)" value="<?php echo JText::_('COM_SOCIALPINBOARD_CREATE'); ?>" />
                        <div class="special" style="display:block;float:left;">

                        </div>
                        <div id="boardError" class="pin-it-error" style="clear:both;"></div></li>
                    <li><div class="PinForm">
                            ...
                            <div class="InputArea">
                                <ul class="Form FancyForm">
                                    <li class="noMarginBottom val">
                                        <textarea class="DescriptionTextarea" style="color:#C9C8C8" name="pin_desc" id="pin_desc" row="3" onFocus="onFocusMenu(this,'<?php echo JText::_('COM_SOCIALPINBOARD_PIN_DESCRIPTION'); ?>');" onBlur="onBlurEvent(this,'<?php echo JText::_('COM_SOCIALPINBOARD_PIN_DESCRIPTION'); ?>');"  value="<?php echo JText::_('COM_SOCIALPINBOARD_PIN_DESCRIPTION'); ?>"><?php echo JText::_('COM_SOCIALPINBOARD_PIN_DESCRIPTION'); ?></textarea>
                                        <div id="pinError"></div>
                                    </li>
                                    <span class="fff"></span>
                            </div>
                    ...
                    <li>
                        ...
                        <input type="hidden" id="uploadPin" name="uploadPin" value="<?php echo JText::_('COM_SOCIALPINBOARD_PIN_IT'); ?>"/>
                        <input type="hidden" id="media_url" name="media_url" value="<?php echo $url; ?>"/>
                        <input type="submit" class="pin-it-submit"name="pinitbtn" id="pinitbtn"  style="display: block" onclick="return pinit('<?php echo $mediaurl; ?>')" value="<?php echo JText::_('COM_SOCIALPINBOARD_PIN_IT'); ?>"/>
                        <strong class="edit-board-btn pinning" id="add_pining" style="display:none;"><?php echo JTEXT::_('COM_SOCAILPINBOARD_PINNING'); ?></strong>
                    </li>
                </ul>
...
<script type="text/javascript">
    ...
    function pinit(url){
        pin_desc=document.getElementById('pin_desc').value;
        document.getElementById('add_pining').style.display="block";
        document.getElementById('pinitbtn').style.display="none";
        if(pin_desc=='<?php echo JText::_('COM_SOCIALPINBOARD_PIN_DESCRIPTION'); ?>')
        {
            alert("<?php echo JText::_('COM_SOCIALPINBOARD_PLEASE_ENTER_DESCRIPTION'); ?>");
            document.getElementById('add_pining').style.display="none";
            document.getElementById('pinitbtn').style.display="block";
            return false;
        }
